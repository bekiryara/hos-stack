...
/**
 * Export brand -> model lists for Arabam "minivan-panelvan".
 *
 * Writes:
 * - catalog/manifests/categories/vehicle-minivan-panelvan-brands.json
 * - catalog/manifests/vehicle-minivan-panelvan-models.arabam.tr.json
 * - catalog/manifests/schema/vehicle-minivan-panelvan-models.json
 *
 * Usage:
 *   node scripts/arabam-export-minivan-panelvan-models.js --write
 */
...
async function main() {
  ...
  const rootRel = "ikinci-el/minivan-panelvan";
  ...
  const catPath = path.join(manifestsRoot, "categories", "vehicle-minivan-panelvan-brands.json");
  fs.writeFileSync(catPath, JSON.stringify(categories, null, 2) + "\n", "utf8");
  console.log("wrote " + catPath);

  const modelPath = path.join(manifestsRoot, "vehicle-minivan-panelvan-models.arabam.tr.json");
