# product_mvp_check.ps1 - Marketplace MVP Loop (Wrapper)
# Historical product MVP loop is retired.
# Current marketplace MVP flow for listings is validated by:
...
Write-Host "=== PRODUCT MVP LOOP (MARKETPLACE WRAPPER) ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Fail "MVP wrapper failed: $($_.Exception.Message)"
    $code = 1
} finally {
