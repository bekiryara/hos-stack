# Demo Seed V1 (WP-69)
# Ensures deterministic demo data for E2E prototype: 3 specific listings
...
Write-Host "=== DEMO SEED V1 (WP-69) ===" -ForegroundColor Cyan
...
# Step 4: Define demo listings (WP-69)
Write-Host ""
Write-Host "[4] Defining demo listings..." -ForegroundColor Yellow
$demoListings = @(
    @{
        Title = "Bando Takimi"
        Description = "Bando presto - Profesyonel bando takimi kiralama"
    ...
    @{
        Title = "Kiralik Tekne"
        Description = "Ruyam - Denizde guzel bir gun gecirmek icin kiralik tekne"
    ...
    @{
        Title = "Adana Kebap"
        Description = "Geleneksel Adana kebap - Lezzetli ve doyurucu"
...
Write-Host "PASS: Demo listings defined ($($demoListings.Count) listings)" -ForegroundColor Green

# Step 5: Ensure each demo listing exists (idempotent by title)
Write-Host ""
Write-Host "[5] Ensuring demo listings exist..." -ForegroundColor Yellow
$results = @()

foreach ($demoListing in $demoListings) {
    $title = $demoListing.Title
    $categorySlug = $demoListing.CategorySlug
    
    Write-Host ""
    Write-Host "  Listing: $title (category: $categorySlug)" -ForegroundColor Cyan
    
    # Find category
    $category = Find-CategoryBySlug -Tree $categories -Slug $categorySlug
    if (-not $category) {
        Write-Host "    WARN: Category '$categorySlug' not found, skipping..." -ForegroundColor Yellow
        continue
    }
    ...
    # Check if listing exists by title (idempotent check)
    $hasListing = $false
    $existingListingId = $null
    
    try {
        $listingsResponse = Invoke-RestMethod -Uri "$pazarBaseUrl/api/v1/listings?category_id=$categoryId&status=published&limit=50" `
            -TimeoutSec 10 `
            -ErrorAction Stop
    ...
    if (-not $hasListing) {
        try {
            ...
            $listingBody = @{
                ...
                transaction_modes = $demoListing.TransactionModes
                attributes = $demoListing.Attributes
            }
    ...
    $results += @{
        ...
        Status = if ($hasListing) { "EXISTS" } else { "CREATED" }
    }
}

# Step 6: Print summary
Write-Host ""
Write-Host "=== DEMO SEED V1 SUMMARY ===" -ForegroundColor Cyan
Write-Host ""

foreach ($result in $results) {
    $statusColor = if ($result.Status -eq "EXISTS") { "Green" } else { "Yellow" }
    Write-Host "[$($result.Status)] $($result.Title) (category: $($result.CategorySlug))" -ForegroundColor $statusColor
...
if ($hasFailures) {
    Write-Host "=== DEMO SEED V1: FAIL ===" -ForegroundColor Red
    exit 1
} else {
    Write-Host "=== DEMO SEED V1: PASS ===" -ForegroundColor Green
    exit 0
}
