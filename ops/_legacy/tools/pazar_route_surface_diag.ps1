...
# Load shared helpers (from canonical ops/_lib)
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$opsDir = Split-Path -Parent (Split-Path -Parent $scriptDir) # tools -> _legacy -> ops
$libDir = Join-Path $opsDir "_lib"
if (Test-Path (Join-Path $libDir "ops_output.ps1")) {
    . (Join-Path $libDir "ops_output.ps1")
    Initialize-OpsOutput
}
if (Test-Path (Join-Path $libDir "ops_exit.ps1")) {
    . (Join-Path $libDir "ops_exit.ps1")
    Initialize-OpsExit
}
if (Test-Path (Join-Path $libDir "routes_json.ps1")) {
    . (Join-Path $libDir "routes_json.ps1")
}
